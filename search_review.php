...
<html>
    ...
    <body>
        <?php
        require_once 'CgiServiceBase.class.php';
        require_once "Review.class.php";
        require_once 'ReviewService.class.php';
        

        class SearchReview extends CgiServiceBase {
            ...
            public function checkInput() {
                ...
                $this->fenyePageReq = new SearchReviewFenyePageReq();
                $this->fenyePageReq->user_id = $user_id;
                $this->fenyePageReq->perPage = 10;
                if (!!isset($_GET['nowPage'])) {
                    $this->fenyePageReq->nowPage = $_GET['nowPage'];
                } else {
                    $this->fenyePageReq->nowPage = 1;
                }
                $this->fenyePageReq->displayPageCount = 10;
                $this->fenyePageReq->goUrl = 'search_review.php';
                if (isset($_GET['keyWord']) && $_GET['keyWord'] != "") {
                    $this->fenyePageReq->keyWord = urldecode($_GET['keyWord']);
            ...
            public function process() {
                ...
                $this->fenyePageRsp = new SearchReviewFenyePageRsp();
                $this->reviewService->searchReviewFenyePage($this->fenyePageReq, $this->fenyePageRsp);
                $row_arr = $this->fenyePageRsp->res;


                $time_stamp_now = time();
                for ($i = 0; $i < count($row_arr); $i++) {
                    $num = ($this->fenyePageReq->nowPage - 1) * $this->fenyePageReq->perPage + $i + 1;
                    $row = $row_arr[$i];
                ...
                //输出导航栏
                echo $this->fenyePageRsp->navigator;
            }

        }

        //开始运行
        $app = new SearchReview();
        $app->run();
        ?>
    </body>
</html>
